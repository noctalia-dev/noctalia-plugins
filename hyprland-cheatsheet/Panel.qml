...
Item {
    ...
    Process {
        id: catProcess
        command: ["sh", "-c", "cat ~/.config/hypr/*.conf ~/.config/hypr/**/*.conf 2>/dev/null"]
        running: false
    ...
    function parseAndSave(text) {
        ...
        if (categories.length > 0) {
            pluginApi.pluginSettings.cheatsheetData = categories;
        ...
        RowLayout {
            ...
            ColumnLayout {
                ...
                Repeater {
                    ...
                    Loader {
                        Layout.fillWidth: true
                        sourceComponent: modelData.type === "header" ? headerComponent : (modelData.type === "spacer" ? spacerComponent : bindComponent)
    ...
    function getKeyColor(keyName) {
        ...
        if (keyName === "Alt")
            return "#FF6B6B"; // Reddish
        if (keyName.startsWith("XF86"))
            return "#4ECDC4"; // Turquoise for multimedia
        if (keyName === "PRINT")
            return "#95E1D3"; // Light turquoise for print screen
        if (keyName.match(/^[0-9]$/))
            return "#A8DADC"; // Light blue for numbers
        if (keyName.includes("MOUSE"))
            return "#F38181"; // Pink for mouse
            ...
            if (cat.binds && cat.binds.length > 12 && cat.title.includes("OBSZARY ROBOCZE")) {
                ...
                for (var j = 0; j < cat.binds.length; j++) {
                    ...
                    if (bind.keys.includes("MOUSE")) {
                        mouse.push(bind);
                    } else if (bind.desc.includes("Send") || bind.desc.includes("send") || bind.desc.includes("Move") || bind.desc.includes("move") || bind.desc.includes("Wyślij") || bind.desc.includes("wyślij")) {
                        moving.push(bind);
                    } else {
                        switching.push(bind);
                    }
